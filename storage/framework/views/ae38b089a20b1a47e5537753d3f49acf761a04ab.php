
<?php $__env->startSection("content"); ?>

    <script src="<?php echo e(asset('resources/OfficeCMS/uploadify/jquery.uploadify.min.js')); ?>" type="text/javascript"></script>
    <link rel="stylesheet" type="text/css" href="<?php echo e(asset('resources/OfficeCMS/uploadify/uploadify.css')); ?>">

    <style>
        .uploadify{display:inline-block;}
        .uploadify-button{border:none; border-radius:5px; margin-top:8px;}
        table.add_tab tr td span.uploadify-button-text{color: #FFF; margin:0;}
    </style>

    <!--面包屑导航 开始-->
    <div class="crumb_warp">
        <!--<i class="fa fa-bell"></i> 欢迎使用登陆网站后台，建站的首选工具。-->
        <i class="fa fa-home"></i> <a href="#">首页</a> &raquo; <a href="#">供应商管理</a> &raquo; 添加供应商
    </div>
    <!--面包屑导航 结束-->

	<!--结果集标题与导航组件 开始-->
	<div class="result_wrap">
        <div class="result_title">
            <h3>快捷操作</h3>
        </div>
        <div class="result_content">
            <div class="short_wrap">
                <a href="javascript:void();" onclick="location.reload();"><i class="fa fa-refresh"></i>更新网页</a>
            </div>
        </div>
    </div>
    <!--结果集标题与导航组件 结束-->
    
    <div class="result_wrap">
        <form action="<?php echo e(url('cms/supplier_storage')); ?>" method="post">
            <?php echo e(csrf_field()); ?>

            <table class="add_tab">
                <tbody>
                    <tr>
                        <th><i class="require">*</i>供应商全称：</th>
                        <td>
                            <input type="text" class="lg" name="fullName">

                            <?php if($errors->has('fullName')): ?>
                                <i class="require"> <?php echo e($errors->first('fullName')); ?></i>
                            <?php else: ?>
                                <span></span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <tr>
                        <th><i class="require">*</i>供应商简称</th>
                        <td>
                            <input type="text" class="lg" name="abbreviation">
                            <?php if($errors->has('abbreviation')): ?>
                                <i class="require"> <?php echo e($errors->first('abbreviation')); ?></i>
                            <?php else: ?>
                                <span></span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <tr>
                        <th><i class="require">*</i>供应商类型：</th>
                        <td>
                            <label><input name="type" type="checkbox" value="0" checked="checked" />厂家</label> 
                            <label><input name="type" type="checkbox" value="" />总代</label> 
                            <label><input name="type" type="checkbox" value="" />经销商</label> 
                            
                            <?php if($errors->has('type')): ?>
                                <i class="require"> <?php echo e($errors->first('type')); ?></i>
                            <?php else: ?>
                            <?php endif; ?>
                        </td>
                    </tr>

                    <tr>
                        <th><i class="require">*</i>供应品牌</th>
                        <td>
                            <input type="text" class="lg" name="brand">
                            <?php if($errors->has('brand')): ?>
                                <i class="require"> <?php echo e($errors->first('brand')); ?></i>
                            <?php else: ?>
                                <span></span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <tr>
                        <th><i class="require">*</i>供应品类</th>
                        <td>
                            <input type="text" class="lg" name="brandType">
                            <?php if($errors->has('brandType')): ?>
                                <i class="require"> <?php echo e($errors->first('brandType')); ?></i>
                            <?php else: ?>
                                <span></span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <tr>
                        <th><i class="require">*</i>办公地址</th>
                        <td>
                            <input type="text" class="lg" name="officeAdd">
                            <?php if($errors->has('officeAdd')): ?>
                                <i class="require"> <?php echo e($errors->first('officeAdd')); ?></i>
                            <?php else: ?>
                                <span></span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <tr>
                        <th><i class="require">*</i>库房地址：</th>
                        <td>
                            <input type="text" class="lg" name="warehoustAdd">
                            <?php if($errors->has('warehoustAdd')): ?>
                                <i class="require"> <?php echo e($errors->first('warehoustAdd')); ?></i>
                            <?php else: ?>
                                <span></span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <tr>
                        <th><i class="require">*</i>采购区域：</th>
                        <td>
                            <input type="text" class="lg" name="area">
                            <?php if($errors->has('area')): ?>
                                <i class="require"> <?php echo e($errors->first('area')); ?></i>
                            <?php else: ?>
                                <span></span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <tr>
                        <th><i class="require">*</i>结算方式：</th>
                        <td>
                            <input type="text" name="settlementMmethod" class="md" >
                            <?php if($errors->has('settlementMmethod')): ?>
                                <i class="require"> <?php echo e($errors->first('settlementMmethod')); ?></i>
                            <?php else: ?>
                                <span>月结、现款、可临时佘帐</span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <tr>
                        <th><i class="require">*</i>收款方式：</th>
                        <td>
                            <input type="text" name="paymentMethod" class="md" >
                            <?php if($errors->has('paymentMethod')): ?>
                                <i class="require"> <?php echo e($errors->first('paymentMethod')); ?></i>
                            <?php else: ?>
                                <span>公帐、卡、现金</span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <tr>
                        <th><i class="require">*</i>结算价格（含税）：</th>
                        <td>
                            <input type="text" class="md" name="priceTax" placeholder="0000.00">元
                            <?php if($errors->has('priceTax')): ?>
                                <i class="require"> <?php echo e($errors->first('priceTax')); ?></i>
                            <?php else: ?>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <tr>
                        <th><i class="require">*</i>结算价格（不含税）：</th>
                        <td>
                            <input type="text" class="md" name="priceNoTax" placeholder="0000.00">元
                            <?php if($errors->has('priceNoTax')): ?>
                                <i class="require"> <?php echo e($errors->first('priceNoTax')); ?></i>
                            <?php else: ?>
                                <span></span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <tr>
                        <th><i class="require">*</i>帐户信息：</th>
                        <td>
                            <input type="text" name="account" class="md" >
                            <?php if($errors->has('account')): ?>
                                <i class="require"> <?php echo e($errors->first('account')); ?></i>
                            <?php else: ?>
                                <span></span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <tr>
                        <th><i class="require">*</i>是否送货：</th>
                        <td>
                            <label><input name="deliveryIs" type="radio" value="0" checked="checked" />是</label> 
                            <label><input name="deliveryIs" type="radio" value="" />否</label> 
                            <?php if($errors->has('deliveryIs')): ?>
                                <i class="require"> <?php echo e($errors->first('deliveryIs')); ?></i>
                            <?php else: ?>
                                <span></span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <tr>
                        <th><i class="require">*</i>是否签协：</th>
                        <td>
                            <label><input name="signIs" type="radio" value="0" checked="checked" />是</label> 
                            <label><input name="signIs" type="radio" value="" />否</label> 
                            <?php if($errors->has('signIs')): ?>
                                <i class="require"> <?php echo e($errors->first('signIs')); ?></i>
                            <?php else: ?>
                                <span></span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <tr>
                        <th>退换货要求：</th>
                        <td>
                            <textarea class="mg" name="returnRequirements"></textarea>
                            <?php if($errors->has('returnRequirements')): ?>
                                <i class="require"> <?php echo e($errors->first('returnRequirements')); ?></i>
                            <?php else: ?>
                                <span></span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <tr>
                        <th><i class="require">*</i>合同到期日：</th>
                        <td>
                            <input type="text" name="contractDate" class="md" placeholder="20126-12-31">
                            <?php if($errors->has('contractDate')): ?>
                                <i class="require"> <?php echo e($errors->first('contractDate')); ?></i>
                            <?php else: ?>
                                <span></span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <tr>
                        <th>合同简报：</th>
                        <td>
                            <textarea class="mg" name="contractBriefing"></textarea>
                            <?php if($errors->has('contractBriefing')): ?>
                                <i class="require"> <?php echo e($errors->first('contractBriefing')); ?></i>
                            <?php else: ?>
                                <span></span>
                            <?php endif; ?>
                        </td>
                    </tr>

                    <tr>
                        <th><i class="require">*</i>授信额度：</th>
                        <td>
                            <input type="text" class="md" name="credit">
                            <?php if($errors->has('credit')): ?>
                                <i class="require"> <?php echo e($errors->first('credit')); ?></i>
                            <?php else: ?>
                                <span></span>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <tr>
                        <th>备注：</th>
                        <td>
                            <textarea class="mg" name="note"></textarea>
                            <?php if($errors->has('note')): ?>
                                <i class="require"> <?php echo e($errors->first('note')); ?></i>
                            <?php else: ?>
                                <span></span>
                            <?php endif; ?>
                        </td>
                    </tr>

                    <tr>
                        <th></th>
                        <td>
                            <input type="submit" value="提交">
                            <input type="button" class="back" onclick="history.go(-1)" value="返回">
                        </td>
                    </tr>
                </tbody>
            </table>
        </form>
    </div>

<?php $__env->stopSection(); ?>
<?php echo $__env->make("officecms.layout.cms", array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>