...
function closeSonWindow(){
    parent.$('.{{$id}}').attr('src',$('#picture').attr('src'));
    parent.$('input[name={{$id}}]').val($('#picture').attr('src'));
    parent.layer.close(index);
    parent.layer.tips('这就是你上传的图片', '#{{$id}}', {time: 3000});
}
        ...
        $(function() {
            $('#file').uploadify({
                ...
                'swf'      : "{{asset('resources/cms/uploadify/uploadify.swf')}}",
