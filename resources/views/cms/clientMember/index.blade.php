...
<div class="page-container">
    ...
    @endif    
            ...
            <a class="btn btn-primary radius" href="{{url('cms/clientMember')}}">清空所有筛选</a>
        </div>
        ...
        <div class="box-shadow">
            ...
            @foreach($whereField as $f=>$lista)
                <a class="btn btn-primary size-MINI radius" href="{{url('cms/clientMember'.$lista['sortUrl'].'&page='.$data->currentPage())}}">{{$lista['name']}}</a>
            @endforeach
    ...
    <div class="cl pd-5 bg-1 bk-gray">
        <span class="l">
            ...
            <a class="btn btn-primary radius" data-title="添加员工" _href="{{url('cms/clientMember/create')}}" onclick="Hui_admin_tab(this)" href="javascript:;">
    ...
    <table class="table table-border table-striped table-bordered table-hover table-bg">
        ...
        <tbody>
            ...
            @if($selectSupplier)
                <tr class="text-c" ondblclick="actionSelectSupplier('{{$list->id}}','{{$list->nameChinese}}');">
            @else
                <tr class="text-c @if($list->close) danger @endif" data-title="{{$list->nameChinese}}" _href="{{url('cms/clientMember')}}/{{$list->id}}/edit" ondblclick="Hui_admin_tab(this)" >
            @endif

            @if($selectSupplier)@else
                <td class="tc"><input type="checkbox" name="id[]" value="{{$list->id}}" selected=""></td>
            @endif
            ...
            @endforeach
                 ...
                 <td @if($list->close && !$selectSupplier) class=" danger " @endif>
                    @if($selectSupplier)
                    <a title="选择" href="javascript:;" onclick="actionSelectSupplier('{{$list->id}}','{{$list->fullName}}');" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe615;</i></a>
                   @else
                    @if($list->close)
...
function actionDelete(id,status){
        ...
        if(status){
            ...
            if(tag){
                ...
                $.post("{{url('cms/clientMember/')}}/"+id,{'_method':'delete','_token':"{{csrf_token()}}",'status':status},function(data){
