...
<nav class="breadcrumb">
    <i class="Hui-iconfont">&#xe67f;</i> <a href="{{url('cms/index/info')}}" >首页 </a><span class="c-gray en">&gt;</span> 
        ...
        编辑：{{$data->name}} 分类 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    <article class="page-container">
        <form action="{{url('cms/classify/'.$data->id)}}" method="post" class="form form-horizontal" id="formClassilyAdd">
            {{csrf_field()}}
                              ...
                              <select class="input-text radius" size="1" name="parentId">
                                ...
                                @foreach($parentData as $list)
                                    <option value="{{$list['id']}}" 
                                        @if($data->parentId==$list->id) selected="selected" @endif 
                                    ><?php $sunNum = substr_count($list['sort'],'-'); for($i=0;$i<$sunNum;$i++){echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";} ?>{{$list['name']}}</option>
                                @endforeach
                              </select>
                            </span>
                    @endif
